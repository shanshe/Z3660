...
volatile uint8_t *Z3660_RTG_BASE=(volatile uint8_t *)0x18000000;
...
*/
//		if( m68k_get_reg(NULL, M68K_REG_A0)==0x4020)
//		if( m68k_get_reg(NULL, M68K_REG_PC)==0xF8024C)
...
extern "C" uint32_t read_autoconfig(uint32_t address)
{
	...
	switch(address&0xFFFF)
	{
		...
		case 0x0004:
			...
			break;
		case 0x0104:
			if((configured&1) == 0 && (enabled&1) == 1) data = 0xAFFFFFFF; // 0b1010 5 for the 256MB Z3 Fast
			else
			if((configured&2) == 0 && (enabled&2) == 2) data = 0xBFFFFFFF; // 0b1011 4 for the RTG PIC
			break;
		case 0x0008:
		...
		case 0x0010: data = 0x9FFFFFFF; break; // manufacturer high byte inverted
		case 0x0110: data = 0x2FFFFFFF; break; //
		case 0x0014: data = 0x9FFFFFFF; break; // manufacturer low byte
		case 0x0114: data = 0x1FFFFFFF; break; //
	...
	switch(address&0xFC)
	{
		case 0x44:
            ...
            if ((configured&2) == 0 && (enabled&2) == 2)
            {
                ...
                {
                	uint32_t address=(0x18000000+j*0x100000UL);
                	setMMU(address,address|NORM_WT_CACHE);      // mapped to 0x18000000
                	setMMU(i*0x100000UL,address|NORM_WT_CACHE); // mapped to 0x18000000
                }
            	setMMU((end-2)*0x100000UL,0x1FE00000|STRONG_ORDERED); // mapped to 0x18000000
//            	setMMU((end-1)*0x100000UL,0x1FF00000|STRONG_ORDERED); // mapped to 0x18000000
                finish_MMU_OP();
                init_rtg_bank();
                // core0 continues
                shared->core0_hold=0;
...
#define CIAAPRA 0xBFE001
	...
	if(address>=autoConfigBaseRTG && address<autoConfigBaseRTG+0x08000000 && (configured&2))
	{
		...
		if(add<0x10000)
		{
			Z3660_RTG_BASE[add+0x20000]=value&0xFF;
...
void m68k_write_memory_16(unsigned int address, unsigned int value)
{
	...
	if(address>=autoConfigBaseRTG && address<autoConfigBaseRTG+0x08000000 && (configured&2))
	{
		...
		if(add<0x10000)
		{
			int add1=add+0x20000;
			*(uint16_t*)(Z3660_RTG_BASE+add1)=swap16(value);
			write_rtg_register(add,value);
			return;
		}
...
void m68k_write_memory_32(unsigned int address, unsigned int value)
{
	...
	if(address>=autoConfigBaseRTG && address<autoConfigBaseRTG+0x08000000 && (configured&2))
	{
		...
		{
			int add1=add+0x20000;
			*(((uint32_t*)(Z3660_RTG_BASE+add1)))=swap32(value);
			write_rtg_register(add,value);
			return;
		}
...
extern "C" uint32_t arm_read_amiga(uint32_t address, uint32_t size)
{
	write_reg(0x08,address);          // address
	write_reg(0x10,0x11|READ_|size);  // command read
	do{
		NOPX;
	}while(read_reg(0x14)==0);        // read ack
	write_reg(0x10,0x01|READ_);       // confirm ack (bit4=0), tristate bus (READ_)
	uint32_t data_read=read_reg(0x1C);
	return(data_read); // read data
}
#else // WRITE_DELAYED
#define NOPX_WRITE \
		NOP;NOP;NOP;NOP;NOP;\
...
extern "C" uint32_t arm_read_amiga(uint32_t address, uint32_t size)
{
	...
	write_reg(0x10,0x11|READ_|size);  // command read
	do{
		write_reg(0x10,0x11|READ_|size);
		//		NOPX;
	}while(read_reg(0x14)==0);        // read ack
	write_reg(0x10,0x01|READ_);       // confirm ack (bit4=0), tristate bus (READ_)
	uint32_t data_read=read_reg(0x1C);
