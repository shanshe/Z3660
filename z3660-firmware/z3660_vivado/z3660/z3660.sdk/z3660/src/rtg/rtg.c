...
void rtg_init(void)
{
	...
	*((uint32_t *)(0x18020000+REG_ZZ_FW_VERSION))= swap32(REVISION_MAJOR << 8 | REVISION_MINOR); // BS
	//    *((uint32_t *)(0x18020000+REG_ZZ_VBLANK_STATUS))= swap32(0); // this is now read directly from FPGA, so this value is not used anymore
	*((uint32_t *)(0x18020000+REG_ZZ_AUDIO_CONFIG))= swap32(1); // AX is present
	*((uint32_t *)(0x18020000+REG_ZZ_DECODER_FIFORX))= swap32(0);
	*((uint32_t *)(0x18020000+REG_ZZ_CPU_FREQ))= swap32(cpu_freq); // FIXME this is for now fixed at 100MHz

	uint8_t* mac = ethernet_get_mac_address_ptr();
	*((uint32_t *)(0x18020000+REG_ZZ_ETH_MAC_HI))= swap32((((uint32_t)mac[0])<<8)|mac[1]);
	*((uint32_t *)(0x18020000+REG_ZZ_ETH_MAC_LO))= swap32((((uint32_t)mac[2])<<24)|(((uint32_t)mac[3])<<16)|(((uint32_t)mac[4])<<8)|mac[5]);

	printf("RTG init...\r\n");
	video_state->framebuffer_pan_offset=0;

	ethernet_send_result = 0;
	*((uint32_t *)(0x18020000+REG_ZZ_ETH_TX))=swap32(ethernet_send_result);
	int frfb=0;
	*((uint32_t *)(0x18020000+REG_ZZ_ETH_RX_ADDRESS))= swap32(RX_BACKLOG_ADDRESS+(frfb<<11)); // <<11 = 2048 (FRAME_SIZE)
...
void other_tasks(void)
{
	...
	if(adc_time_counter==100000)
		*((uint32_t *)(0x18020000+REG_ZZ_TEMPERATURE))= swap32((uint32_t)(xadc_get_temperature()*10.0));
	if(adc_time_counter==200000)
		*((uint32_t *)(0x18020000+REG_ZZ_VOLTAGE_AUX))= swap32((uint32_t)(xadc_get_aux_voltage()*100.0));
	if(adc_time_counter==300000)
	{
		*((uint32_t *)(0x18020000+REG_ZZ_VOLTAGE_INT))= swap32((uint32_t)(xadc_get_int_voltage()*100.0));
		adc_time_counter=0;
	}
	{
		...
		if(last_interrupt != current_interrupt)
		{
			...
			*((uint32_t *)(0x18020000+REG_ZZ_INT_STATUS))= swap32(last_interrupt);
		}
	}
...
void rtg_loop(void)
{
	...
	if((gpio&0x80000000)!=0)
	{
		...
		address=(uint32_t *)((gpio&0xFFFF) + 0x18020000);
...
void write_rtg_register(uint16_t zaddr,uint32_t zdata)
{
	...
	case REG_ZZ_CONFIG:
		...
		*((uint32_t *)(0x18020000+REG_ZZ_INT_STATUS))= swap32(amiga_interrupt_get());
		break;
	case REG_ZZ_MODE: {
...
*/
	case REG_ZZ_SET_FEATURE:
		...
		case REG_ZZ_AUDIO_SWAB:
		{
			...
			*((uint32_t *)(0x18020000+REG_ZZ_AUDIO_SWAB))= swap32(audio_buffer_collision);
		...
		case REG_ZZ_AUDIO_PARAM:
			...
			if (zdata<ZZ_NUM_AUDIO_PARAMS) {
				audio_param = zdata;
				*((uint32_t *)(0x18020000+REG_ZZ_AUDIO_VAL))= swap32(audio_params[audio_param]); // read param
			} else {
				audio_param = -1;
			}
			break;
			...
			if(audio_param>=0) {
				...
				if (audio_param == AP_TX_BUF_OFFS) {
					...
					if (((uint32_t)addr)>=(AUDIO_TX_BUFFER_ADDRESS*0+0x06000000) && ((uint32_t)addr)<TX_BD_LIST_START_ADDRESS) {
						audio_set_tx_buffer(0x18000000+addr);
					...
					/*
							...
							if (((uint32_t)addr)>=(AUDIO_RX_BUFFER_ADDRESS*0+0x06000000) && ((uint32_t)addr)<TX_BD_LIST_START_ADDRESS) {
								audio_set_rx_buffer((0x18000000+addr);
								audio_request_init = 1;
							} else {
		...
		case REG_ZZ_DECODE:
		{
			...
			uint8_t* input_buffer = (uint8_t*)(0x18000000+decoder_params[0]);

			size_t input_buffer_size = decoder_params[1];

			uint8_t* output_buffer = (uint8_t*)(0x18000000+decoder_params[2]);
			...
			case DECODE_RUN: {
				...
				//									output_buffer, output_buffer_size,fifo_get_read_index(),swap32(*((uint32_t *)(0x18020000+REG_ZZ_DECODER_FIFOTX))));
			}
			break;
			}
			*((uint32_t *)(0x18020000+REG_ZZ_DECODER_FIFORX))= swap32(fifo_get_read_index());
			//					printf("[fifo:fiforx:%d]\n",fifo_get_read_index());
			// this is used only by axmp3 app
			*((uint32_t *)(0x18020000+REG_ZZ_DECODE))= swap32(decoder_bytes_decoded);
			break;
		}
		/*
