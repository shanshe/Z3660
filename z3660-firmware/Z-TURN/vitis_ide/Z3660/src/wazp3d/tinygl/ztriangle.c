...
{						\
    ...
    if (ZCMP(zz,pz[_a])) {				\
      tmp=rgb & 0xF81F07E0;			\
      pp[_a]=__builtin_bswap16(tmp | (tmp >> 16));			\
      pz[_a]=zz;				\
    }						\
    z+=dzdx;					\
...
{						\
   zz=z >> ZB_POINT_Z_FRAC_BITS;		\
     if (ZCMP(zz,pz[_a])) {				\
       pp[_a]=__builtin_bswap16(texture[((t & 0x3FC00000) | s) >> 14]);	\
       pz[_a]=zz;				\
    }						\
    z+=dzdx;					\
    s+=dsdx;					\
...
{						\
   zz=z >> ZB_POINT_Z_FRAC_BITS;		\
     if (ZCMP(zz,pz[_a])) {				\
       pp[_a]=__builtin_bswap16(*(PIXEL *)((char *)texture+ \
