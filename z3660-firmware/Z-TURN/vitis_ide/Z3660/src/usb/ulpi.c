...
int ulpi_init(struct ulpi_viewport *ulpi_vp) {
    ...
    // Leer registros de ID ULPI
    val = ulpi_read(ulpi_vp, (u8 *)3);
    if (val == ULPI_ERROR)
    {
       printf("Error reading ULPI\n");
       return val;
    }
    id = (id << 8) | val;
    val = ulpi_read(ulpi_vp, (u8 *)2);
    if (val == ULPI_ERROR)
    {
       printf("Error reading ULPI\n");
       return val;
    ...
    int integrity_check = ulpi_integrity_check(ulpi_vp);
    if (integrity_check != 0) {
        return integrity_check;
    }

    // Configurar el reloj del transceiver para 24 MHz si es necesario
    val = ulpi_read(ulpi_vp, &ulpi->function_ctrl);
    if (val != ULPI_ERROR) {
        // Configura el valor de velocidad si es aplicable
        val &= ~ULPI_FC_XCVRSEL_MASK;
        val &= ~ULPI_FC_OPMODE_MASK;
        val |= ULPI_FC_OPMODE_NORMAL; // Asegurar que está en modo normal
        ulpi_write(ulpi_vp, &ulpi->function_ctrl, val);
    }
