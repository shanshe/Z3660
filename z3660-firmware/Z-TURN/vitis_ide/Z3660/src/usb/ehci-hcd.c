...
#define CONFIG_USB_EHCI_TXFIFO_THRESH 4
...
//#endif
   ...
   if (QT_TOKEN_GET_STATUS(token) & QT_TOKEN_STATUS_ACTIVE) {
		...
		printf("[ehci-dbg] dev: addr=%d speed=%d maxpkt=%d ep=%ld dir=%s\n",
                ...
                struct devrequest *setup)
{
   ...
   if((int)usb_pipedevice(pipe) == ctrl->rootdev) {
      if (!ctrl->rootdev) {
          dev->speed = USB_SPEED_HIGH;
          /* Mantener la velocidad detectada por el hardware */
          printf("[ehci] Root device enumeration, speed=%d\n", dev->speed);
      }
