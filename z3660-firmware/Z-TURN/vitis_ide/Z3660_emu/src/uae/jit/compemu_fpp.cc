...
#define debug_compemu_fpp(...)
//#define debug_compemu_fpp printf
...
void comp_fscc_opp (uae_u32 opcode, uae_u16 extra)
{
   ...
   if (!currprefs.compfpu) {
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fscc_opp currprefs.compfpu\n");
      return;
   }

   if (extra & 0x20) {  /* only cc from 00 to 1f are defined */
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fscc_opp only cc from 00 to 1f are defined\n");
      return;
   }
   if ((opcode & 0x38) != 0) { /* We can only do to integer register */
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP]  comp_fscc_opp We can only do to integer register\n");
      return;
   }
...
void comp_fbcc_opp (uae_u32 opcode)
{
   ...
   if (!currprefs.compfpu) {
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fbcc_opp currprefs.compfpu\n");
      return;
   }

   if (opcode & 0x20) {  /* only cc from 00 to 1f are defined */
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fbcc_opp only cc from 00 to 1f are defined\n");
      return;
   }
   if (!(opcode & 0x40)) {
      off = (uae_s32) (uae_s16) comp_get_iword ((m68k_pc_offset += 2) - 2);
...
void comp_fpp_opp (uae_u32 opcode, uae_u16 extra)
{
   ...
   if (special_mem) {
      FAIL(1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp special_mem\n");
      return;
   }

   if (!currprefs.compfpu) {
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp currprefs.compfpu\n");
      return;
   }
   switch (source) {
   case 3: /* FMOVE FPx, <EA> */
      ...
      {
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp comp_fp_put\n");
      }
      return;
   case 4: /* FMOVE.L  <EA>, ControlReg */
      if ((opcode & 0x38) == 0) {
         ...
         if (bits && bits != 0x1000 && bits != 0x0800 && bits != 0x400) {
            FAIL(1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L Only single selected control register is allowed\n");
            return;
         }
         if (extra & 0x1000) { /* FPCR */
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L FPCR\n");
            return;
         }
         if (extra & 0x0800) { /* FPSR */
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L FPSR\n");
            return;
         }
         ...
         if (bits && bits != 0x0400) {
            FAIL(1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L Only FPIAR can be moved to/from address register\n");
            return;
         }
         mov_l_mr ((uintptr)&regs.fpiar, opcode & 15);
         return;
      ...
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L\n");
      return;
   case 5: /* FMOVE.L  ControlReg, <EA> */
      if ((opcode & 0x38) == 0) {
         ...
         if (bits && bits != 0x1000 && bits != 0x0800 && bits != 0x400) {
            FAIL(1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L case 5 Only single selected control register is allowed\n");
            return;
         }
         if (extra & 0x1000) /* FPCR */
         ...
         if (extra & 0x0800) { /* FPSR */
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L case 5 FPSR\n");
            return;
         }
         if ((extra & 0x0400) || !bits) /* FPIAR */
         ...
         if (bits && bits != 0x0400) {
            FAIL(1);
            debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L case 5 Only FPIAR can be moved to/from address register\n");
            return;
         }
         mov_l_rm (opcode & 15, (uintptr)&regs.fpiar);
         return;
      ...
      FAIL (1);
      debug_compemu_fpp("[COMPEMU_FPP] comp_fpp_opp FMOVE.L case 5\n");
      return;
   ...
   case 7:
   {
      ...
      if (extra & 0x2000) {
         ...
         default:
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] FMOVEM FPP->memory\n");
            return;
         }
         ...
         default:
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] FMOVEM memory->FPP\n");
            return;
         }
         ad = comp_fp_adr (opcode);
         if (ad < 0) {
   ...
   case 2: /* from <EA> to FPx */
      ...
      if ((extra & 0xfc00) == 0x5c00) { /* FMOVECR */
         ...
         default:
            FAIL (1);
            debug_compemu_fpp("[COMPEMU_FPP] from <EA> to FPx\n");
            return;
         }
      ...
      if(opmode >= 0x30 && opmode <= 0x37) {
         ...
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] get out early for unsupported ops\n");
         return;
      }
      if ((prec = comp_fp_get (opcode, extra, sreg)) < 0) {
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] comp_fp_get\n");
         return;
      }
   ...
   case 0: /* directly from sreg to dreg */
      ...
      case 0x37:
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] FSINCOS\n");
         return;
      case 0x38: /* FCMP */
         fmov_rr (FP_RESULT, dreg);
      ...
      default:
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] case 0\n");
         return;
      }
      fmov_rr (FP_RESULT, dreg);
      return;
      default:
         ...
         printf (_T ("Unsupported JIT-FPU instruction: 0x%04x %04x\n"), opcode, extra);
         FAIL (1);
         debug_compemu_fpp("[COMPEMU_FPP] Unsupported JIT-FPU instruction: 0x%04x %04x\n", opcode, extra);
         return;
   }
}
#endif
