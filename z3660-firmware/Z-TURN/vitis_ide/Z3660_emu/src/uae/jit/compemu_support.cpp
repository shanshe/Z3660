...
// or Exec (but never both at the same time)
  ...
  return (uae_u8 *) cache;
   ...
   if (!cache) {
      ...
      memset(cache, 0, size);
      printf ("Cache_Alloc of %d at 0x%08X.\n",size,(uint32_t)cache);
   }
   return (uae_u8 *) cache;
}
 ...
 * into the regs.ccrflags.nzcv of the traditional emulation. Thus some odd
 * names.
 *
...
#elif defined(CPU_arm)
#include <jit/compemu_midfunc_arm.cpp>
#include <jit/compemu_midfunc_arm2.cpp>
#endif
...
{
   if(!isconst(r)) {
      printf("Register %d should be constant, but isn't", r);
   }
...
{
   // if(ovl==1 && addr<0x00080000)
   // {
   //    return(1);
   // }
   if(addr>=0x00f80000 && addr<0x1000000)
   {
      return(1);
   }
   return(0);
...
STATIC_INLINE void writemem_real(int address, int source, int size)
{
   if (0) {
...
STATIC_INLINE void readmem_real(int address, int dest, int size)
{
   if (0) {
      //    if (currprefs.address_space_24) {
      switch (size) {
...
{
   // FIXME: There is something wrong with direct read word (jnf_MEM_READ_OFF_w)
   // It is NOT the readword, it is some JIT compiler behaviour
   if (special_mem & S_READ)
...
#ifdef JIT_DEBUG
   write_log("Address of regs: 0x%08x, regs.pc_p: 0x%08x\n", &regs, &regs.pc_p);
   write_log("Address of natmem_offset: 0x%08x, natmem_offset = 0x%08x\n", &regs.natmem_offset, regs.natmem_offset);
   write_log("Address of cache_tags: 0x%08x\n", cache_tags);
#endif
...
for (i = 0; tbl[i].opcode < 65536; i++) {
   ...
   //#define DISABLE_ALL_OPCODES
#ifdef DISABLE_ALL_OPCODES
   compfunctbl[tbl[i].opcode] = NULL;
#else
   //      bool uses_fpu = (tbl[i].specific & COMP_OPCODE_USES_FPU) != 0;
   //      if (uses_fpu && avoid_fpu)
   if(  ( ((tbl[i].specific & COMP_BAD) != 0) )
         //        || ( ((tbl[i].specific & COMP_OPCODE_LONG_OPCODE) != 0)
         //             && 0 && (tbl[i].opcode>=0x4C00) && (tbl[i].opcode<0x4C80) // de 4C00 a 4C40 esta la sombra en shapeshifter
         //           )
         //        || ( ((tbl[i].specific & COMP_OPCODE_LONG_OPCODE) == 0)
         //             && 0 && (tbl[i].opcode>=0x8100) && (tbl[i].opcode<0x8200) // de 8100 a 8200 esta el jumpy cursor de shapeshifter
         //           )
         //            || ( (tbl[i].opcode>=0x2000) && (tbl[i].opcode<0x4000) )
   )
   {
      compfunctbl[tbl[i].opcode] = NULL;
      int find=-1;
      for(int j=0;;j++)
      {
         if(lookuptab[j].mnemo==table68k[tbl[i].opcode].mnemo)
         {
            find=j;
            break;
         }
      }
      printf("op_%x_0_comp_ff %s 0x%08X\n",tbl[i].opcode,find>=0?lookuptab[find].name:"NO NAME OPCODE!!!!",tbl[i].specific);
   }
   else
   {
...
for (i = 0; nftbl[i].opcode < 65536; i++) {
   ...
   //      if (uses_fpu && avoid_fpu)
#ifdef DISABLE_ALL_OPCODES
   compfunctbl[nftbl[i].opcode] = NULL;
#else
   if(  ( ((nftbl[i].specific & COMP_BAD) != 0) )
         //        || ( ((nftbl[i].specific & COMP_OPCODE_LONG_OPCODE) != 0)
         //          || ( (nftbl[i].opcode>=0x8000) && (nftbl[i].opcode<0xFF00) )
         //           )
   )
   {
      ...
      for(int j=0;;j++)
      {
         if(lookuptab[j].mnemo==table68k[nftbl[i].opcode].mnemo)
         {
            find=j;
            break;
         }
      }
      printf("op_%x_0_comp_nf %s 0x%08X\n",nftbl[i].opcode,find>=0?lookuptab[find].name:"NO NAME OPCODE!!!!",nftbl[i].specific);
   }
   else
   {
...
// raw_cputbl_count[] is indexed with plain opcode (in m68k order)
   ...
   may_raise_exception = false;
         ...
         if (next_pc_p) { /* A branch was registered */
            ...
            blockinfo* tbi;
#ifdef JIT_DEBUG
            write_log("    branch detected: t1=0x" PRINT_PTR ", t2=0x" PRINT_PTR ", cc=%d\n", t1, t2, cc);
#endif
