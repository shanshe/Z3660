...
#define SET_ZFLG(y) (regs.ccrflags.cznv = (((uae_u32)regs.ccrflags.cznv) & ~FLAGVAL_Z) | (((y) & 1) << FLAGBIT_Z))
#define SET_CFLG(y) (regs.ccrflags.cznv = (((uae_u32)regs.ccrflags.cznv) & ~FLAGVAL_C) | (((y) & 1) << FLAGBIT_C))
#define SET_VFLG(y) (regs.ccrflags.cznv = (((uae_u32)regs.ccrflags.cznv) & ~FLAGVAL_V) | (((y) & 1) << FLAGBIT_V))
#define SET_NFLG(y) (regs.ccrflags.cznv = (((uae_u32)regs.ccrflags.cznv) & ~FLAGVAL_N) | (((y) & 1) << FLAGBIT_N))
#define SET_XFLG(y) (regs.ccrflags.x = ((y) & 1) << FLAGBIT_X)
...
#define COPY_CARRY() (regs.ccrflags.x = regs.ccrflags.cznv >> (FLAGBIT_C - FLAGBIT_X))
...
#define optflag_testl(v) \
  __asm__ __volatile__ ("andl %1,%1\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv) : "r" (v) : "memory", "cc")
...
#define optflag_testb(v) \
  __asm__ __volatile__ ("andb %b1,%b1\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv) : "q" (v) : "memory", "cc")

#define optflag_addl(v, s, d) do { \
  __asm__ __volatile__ ("addl %k2,%k1\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv), "=r" (v) : "rmi" (s), "1" (d) : "memory", "cc"); \
    COPY_CARRY(); \
    } while (0)
...
#define optflag_addb(v, s, d) do { \
  __asm__ __volatile__ ("addb %b2,%b1\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=r" (regs.ccrflags.cznv), "=q" (v) : "qmi" (s), "1" (d) : "cc"); \
    COPY_CARRY(); \
    } while (0)
...
#define optflag_subb(v, s, d) do { \
  __asm__ __volatile__ ("subb %b2,%b1\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv), "=q" (v) : "qmi" (s), "1" (d) : "memory", "cc"); \
    COPY_CARRY(); \
    } while (0)
...
#define optflag_cmpl(s, d) \
  __asm__ __volatile__ ("cmpl %k1,%k2\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv) : "rmi" (s), "r" (d) : "memory", "cc")
...
#define optflag_cmpb(s, d) \
  __asm__ __volatile__ ("cmpb %b1,%b2\n\t" \
         "pushf\n\t" \
         "pop %0\n\t" \
         : "=rm" (regs.ccrflags.cznv) : "qmi" (s), "q" (d) : "memory", "cc")
...
#define optflag_testl(v) \
  __asm__ __volatile__ ("andl %0,%0\n\t" \
         ASM_LAHF "\n\t" \
         "seto %%al\n\t" \
         "movb %%al,regs.ccrflags\n\t" \
         "movb %%ah,regs.ccrflags+1\n\t" \
         : : "r" (v) : "%eax","cc","memory")
...
#define SET_NFLG(y)     (regs.ccrflags.nzcv = (regs.ccrflags.nzcv & ~FLAGVAL_N) | (((y) ? 1 : 0) << FLAGBIT_N))
#define SET_ZFLG(y)     (regs.ccrflags.nzcv = (regs.ccrflags.nzcv & ~FLAGVAL_Z) | (((y) ? 1 : 0) << FLAGBIT_Z))
#define SET_CFLG(y)     (regs.ccrflags.nzcv = (regs.ccrflags.nzcv & ~FLAGVAL_C) | (((y) ? 1 : 0) << FLAGBIT_C))
#define SET_VFLG(y)     (regs.ccrflags.nzcv = (regs.ccrflags.nzcv & ~FLAGVAL_V) | (((y) ? 1 : 0) << FLAGBIT_V))
#define SET_XFLG(y)     (regs.ccrflags.x    = (regs.ccrflags.x    & ~FLAGVAL_X) | (((y) ? 1 : 0) << FLAGBIT_X))

#define GET_NFLG()      ((regs.ccrflags.nzcv >> FLAGBIT_N) & 1)
#define GET_ZFLG()      ((regs.ccrflags.nzcv >> FLAGBIT_Z) & 1)
#define GET_CFLG()      ((regs.ccrflags.nzcv >> FLAGBIT_C) & 1)
#define GET_VFLG()      ((regs.ccrflags.nzcv >> FLAGBIT_V) & 1)
#define GET_XFLG()      ((regs.ccrflags.x    >> FLAGBIT_X) & 1)
...
#define optflag_addl(v, s, d) do { \
  __asm__ __volatile__ ("adds %[rv],%[rd],%[rs]\n\t" \
                        "mrs %[nzcv],cpsr\n\t" \
                        : [nzcv] "=r" (regs.ccrflags.nzcv), [rv] "=r" (v) \
                        : [rs] "ri" (s), [rd] "1" (d) \
                        : "cc"); \
    COPY_CARRY(); \
    } while(0)
