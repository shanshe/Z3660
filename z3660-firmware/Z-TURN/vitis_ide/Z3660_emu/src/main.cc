...
extern "C" void write_scsi_register(uint16_t zaddr,uint32_t zdata,int type)
{
	...
	last_type1=type;
	shared->write_scsi=1;
/*	if((zaddr&0xFFFF)==PISCSI_CMD_ADDR2)
		addr2=zdata;
	if((zaddr&0xFFFF)==PISCSI_CMD_ADDR3)
		addr3=zdata;
	if(   ((zaddr&0xFFFF)==PISCSI_CMD_WRITE)
		||((zaddr&0xFFFF)==PISCSI_CMD_WRITEBYTES)
		||((zaddr&0xFFFF)==PISCSI_CMD_WRITE64) )
		Xil_DCacheFlushRange(addr3,addr2);
	if(   ((zaddr&0xFFFF)==PISCSI_CMD_READ)
		||((zaddr&0xFFFF)==PISCSI_CMD_READBYTES)
		||((zaddr&0xFFFF)==PISCSI_CMD_READ64) )
		Xil_DCacheFlushRange(addr3,addr2);*/
	shared->shared_data=1;
