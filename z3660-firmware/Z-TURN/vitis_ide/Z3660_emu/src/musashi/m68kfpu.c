...
//				source = READ_EA_FPE(imode, reg, di_mode_ea);
	...
	switch (opmode)
	{
		...
		{
			REG_FP[dst] = floatx80_sin(source, &status);
			REG_FP[w2&7] = floatx80_cos(source, &status);
			SET_CONDITION_CODES(REG_FP[dst]);    // condition codes are set for the sine result
			USE_CYCLES(451);
			break;
		}
		case 0x38:		// FCMP
