...
double zz_get_ltc_resistor(void)
{
	double ltc_r = (double)(zz_get_reg(REG_ZZ_LTC_RESISTOR));
	return ltc_r;
}
...
void refresh_zz_info(struct Window* win)
{
	...
	double ltc_temp = zz_get_ltc_temperature();
	double ltc_v1 = zz_get_ltc_v1();
	double ltc_v2 = zz_get_ltc_v2();
	double ltc_r = zz_get_ltc_resistor();
...
//										TAG_END);
	...
	ng.ng_GadgetID   = MYGAD_LTC_R;
	ng.ng_GadgetText = (STRPTR)"LTC (THERM) Ohms";

	gads[MYGAD_LTC_R] = gad = CreateGadget(STRING_KIND, gad, &ng,
										GTST_String, "",
										TAG_END);
