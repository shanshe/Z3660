...
/* we can use INT2 but with a z3660 board mod (default is INT6)
	BPTR fh;
		      ...
		      ASMR(a6) DEVBASEP ASMREG(a6))
{
	...
	if (unit == 0 && db->db_Lib.lib_OpenCnt == 1) {
		if ((bm = (struct BufferManagement *)AllocVec(sizeof(struct BufferManagement), MEMF_CLEAR | MEMF_PUBLIC))) {
			bm->bm_CopyToBuffer =
			    (BMFunc)GetTagData(S2_CopyToBuff, 0, (struct TagItem *)ioreq-> ios2_BufferManagement);
			bm->bm_CopyFromBuffer =
			    (BMFunc)GetTagData(S2_CopyFromBuff, 0, (struct TagItem *)ioreq-> ios2_BufferManagement);
...
SAVEDS BPTR DevExpunge(ASMR(a6) DEVBASEP ASMREG(a6))
{
	...
	CloseLibrary(DOSBase);
	CloseLibrary(UtilityBase);
	FreeMem(((BYTE *) db) - db->db_Lib.lib_NegSize,
		(ULONG) (db->db_Lib.lib_PosSize + db->db_Lib.lib_NegSize));
			 ...
			 ASMR(a6) DEVBASEP ASMREG(a6))
{
	...
	case CMD_READ:
		if (ioreq->ios2_BufferManagement == NULL) {
			...
			AddHead((struct List *)&db->db_ReadList,
				(struct Node *)ioreq);
	...
	case S2_DEVICEQUERY:
		{
			...
			devquery->SizeSupplied =
			    (devquery->SizeAvailable <
			     30 ? devquery->SizeAvailable : 30);
		}
		break;
	case S2_GETGLOBALSTATS:
		{
			if (ioreq->ios2_StatData)
				memcpy(ioreq->ios2_StatData, &global_stats,
				       sizeof(struct Sana2DeviceStats));
		}
	case S2_GETSPECIALSTATS:
		{
			struct Sana2SpecialStatHeader *s2ssh =
